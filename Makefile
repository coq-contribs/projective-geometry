...
VFILES:=Space/desargues2Dlemmas2.v\
...
uninstall_me.sh:
	echo '#!/bin/sh' > $@ 
	printf 'cd "$${DSTROOT}"$(COQLIBINSTALL)/ProjectiveGeometry && rm -f $(VOFILES) $(CMOFILES) $(CMIFILES) $(CMAFILES) && find . -type d -and -empty -delete\ncd "$${DSTROOT}"$(COQLIBINSTALL) && find "ProjectiveGeometry" -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
