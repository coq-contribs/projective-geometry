...
let a1_exist a b =
  match a with
    | P2 (x, x0) ->
        (match b with
           | P2 (x1, x2) ->
               (match Top.coq_F_eq_dec x2 x0 with
                  | Left -> general_line_through (P2 (x, x0)) (P2 (x1, x2))
                  | Right ->
                      (match Top.coq_F_eq_dec x0 f0 with
                         | Left -> L2 (f0, (fdiv (fopp f1) x0))
                         | Right -> L1 f0))
           | P1 x1 -> general_line_through (P2 (x, x0)) (P1 x1)
           | P0 -> general_line_through (P2 (x, x0)) P0)
    | P1 x ->
        (match b with
           | P2 (x0, x1) -> general_line_through (P1 x) (P2 (x0, x1))
           | _ -> L2 (f0, f0))
    | P0 ->
        (match b with
           | P2 (x, x0) -> general_line_through P0 (P2 (x, x0))
           | _ -> L2 (f0, f0))
*)
...
IsoleVar r0 H2.
subst.
simplify_eqs H0 H1 H3 H0.
IsoleVar r3 H1.
subst.
ring_simplify in H0.
replace (r * r2 * r4 * r1) with (r * r2 * (r1 * r4)) in H0 by ring.
IsoleVar (r1 * r4) H3.
rewrite H3 in H0.
ring_simplify in H0.
intuition.
simplify_eqs H3 H2 H1 H0.
intuition.
simplify_eqs H0 H3 H1 H2.
IsoleVar r4 H3.
subst.
IsoleVar r0 H0.
subst.
ring_simplify in H2.
IsoleVar (r1 * r2)  H1.
replace (r*r3*r2 * r1) with (r*r3*(r1 * r2)) in H2 by ring.
rewrite H1 in H2.
ring_simplify in H2.
intuition.
Time gb.
...
intuition.
Time gb.
simplify_eqs H2 H0 H3 H1.
IsoleVar r3 H0.
subst.
IsoleVar r1 H3.
subst.
ring_simplify in H1.
replace ( r0 * r2 * r4 * r) with (r0 * r2 *(r *r4)) in H1 by ring.
IsoleVar (r * r4) H2.
rewrite H2 in H1.
ring_simplify in H1.
intuition.
...
subst.
ring_simplify in H3.
replace (r0 * r3 * r2 * r) with (r0*r3*(r*r2)) in H3 by ring.
IsoleVar (r*r2) H0.
rewrite H0 in  H3.
ring_simplify in H3.
intuition.
...
Time gb.
Time gb.
simplify_eqs H2 H1 H3 H0.
ring.
Time gb.
simplify_eqs H2 H1 H3 H0.
ring.
Time gb.
simplify_eqs H1 H3 H0 H2.
intuition.
simplify_eqs H1 H3 H0 H2.
intuition.
simplify_eqs H1 H3 H0 H2.
