...
let rec vars=function
    Pint _->[]
  |Prec (x,l)->(merge ([x]::(List.map vars (Array.to_list l))))
...
let rec string_of_P=function
  |Pint a->if le_coef coef0 a
    then string_of_coef a
    else "("^(string_of_coef a)^")"
  |Prec (x,t)->let v=string_of_var x and s=ref "" and sp=ref "" in 
      if (string_of_P t.(0))<>"0" then s:=string_of_P t.(0);
      for i=1 to (Array.length t)-1 do
        let si=string_of_P t.(i) in
          sp:="";
          if i=1 then 
            (if si<>"0"
	     then (if si="1"
...
and div_pol p q x =
  let (s,r) = quo_rem_pol p q x in
    if eqP r cf0 then s else failwith "div_pol2"
