...
Ltac new_case P :=match (type of P) with Point =>  
generalize (is_only_7_pts P) ; let Q := fresh in (intros Q; decompose [sumor sumbool] Q; clear Q; subst)
| Line =>generalize (is_only_7_lines P) ; let Q := fresh in (intros Q; decompose [sumor sumbool] Q; clear Q; subst)
end.

Lemma col_degen_1 : forall P R, 
~ collinear P P R -> False.
Proof.
intros.
apply H.
unfold collinear in *.
...
H:~collinear ?P ?Q ?R |- _ => 
  solve [
assert (collinear P Q R) by 
(first [solve_ex ABF | solve_ex BCD |  solve_ex CAE |  solve_ex ADG |  solve_ex BEG |  solve_ex CFG |  solve_ex DEF ]); 
